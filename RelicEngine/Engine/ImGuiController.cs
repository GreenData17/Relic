...
namespace Relic.Engine
{
    public class ImGuiController : IDisposable
    {
        ...
        public unsafe ImGuiController(int width, int height)
        {
            ...
            mainViewport.PlatformUserData = (IntPtr)GCHandle.Alloc(this);
            ...
            //io.BackendFlags |= ImGuiBackendFlags.PlatformHasViewports; // Have windows outside of main (does not work right now)
        ...
        private static void altRecreateFontDeviceTexture()
        {
            ImGuiIOPtr io = ImGui.GetIO(); IntPtr pixels;
            int width, height, bytesPerPixel;
            io.Fonts.GetTexDataAsRGBA32(out pixels, out width, out height, out bytesPerPixel);
            io.Fonts.ClearTexData();
        }

        unsafe void SetPlatform()
        {
            var viewport = ImGui.GetWindowViewport();
            viewport.PlatformHandle = GLFW.GetWin32Window(GLFW.GetCurrentContext());
        }
    ...
    outputColor = color * texture(in_fontTexture, texCoord);
        ...
        private void RenderImDrawData(ImDrawDataPtr draw_data)
        {
            ...
            for (int i = 0; i < draw_data.CmdListsCount; i++)
            {
                ...
                if (vertexSize > _vertexBufferSize)
                {
                    ...
                    Debug.LogEngine($"Resized dear imgui vertex buffer to new size {_vertexBufferSize}");
                }
                ...
                if (indexSize > _indexBufferSize)
                {
                    ...
                    Debug.LogEngine($"Resized dear imgui index buffer to new size {_indexBufferSize}");
                }
            }
        ...
        public static int CreateProgram(string name, string vertexSource, string fragmentSoruce)
        {
            ...
            if (success == 0)
            {
                ...
                Debug.LogEngine($"GL.LinkProgram had info log [{name}]:\n{info}");
            }
        ...
        private static int CompileShader(string name, ShaderType type, string source)
        {
            ...
            if (success == 0)
            {
                ...
                Debug.LogEngine($"GL.CompileShader for shader '{name}' [{type}] had info log:\n{info}");
            }
